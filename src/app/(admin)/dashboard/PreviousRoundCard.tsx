...
export default function PreviousRoundCard({ roundNumber, drawDate, winningNumbers, bonusNumber }: Props) {
  return (
    <div className="bg-card rounded-xl border border-border p-5 flex flex-col gap-4">
      <p className="text-xs font-semibold text-muted-foreground tracking-tight uppercase">최근 추첨 결과</p>
      <div>
        <p className="text-2xl font-bold tabular-nums tracking-tight text-foreground">제{roundNumber}회</p>
        <p className="text-xs text-muted-foreground tracking-tight mt-1">{formatKSTDate(drawDate)} 추첨</p>
      </div>
      {winningNumbers && winningNumbers.length > 0 ? (
        <div className="flex items-end gap-3">
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-xs text-muted-foreground tracking-tight text-center">당첨번호</p>
          </div>
          {bonusNumber != null && (
            <>
              ...
              <div className="flex flex-col gap-1.5 items-center">
                ...
                <p className="text-xs text-muted-foreground tracking-tight">보너스번호</p>
              </div>
            </>
          )}
        </div>
      ) : (
        <p className="text-sm text-muted-foreground tracking-tight">결과 대기중</p>
      )}
    </div>
  )
}
